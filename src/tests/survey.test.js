...
describe("Survey API", () => {
	...
	describe("GET /api/surveys/pending", () => {
		beforeEach(async () => {
			// Create a completed appointment without a survey
			await TestUtils.createTestAppointment(
				patient.patient.id,
				doctor.doctor.id,
				{
					status: "completed",
		...
		it("should get pending surveys for patient", async () => {
			const response = await request(app)
			...
			const pendingSurvey = response.body[0];
			expect(pendingSurvey).toHaveProperty("appointmentId");
			expect(pendingSurvey).toHaveProperty("doctorName");
			expect(pendingSurvey).toHaveProperty("specialty");
			expect(pendingSurvey).toHaveProperty("appointmentDate");
			expect(pendingSurvey).toHaveProperty("daysSinceCompletion");
			expect(pendingSurvey).toHaveProperty("formattedDate");
		});

		it("should not return appointments that already have surveys", async () => {
			// Create a survey for the appointment
			await TestUtils.createTestSurvey(patient.patient.id, {
				appointment_id: testAppointment.id,
			});
			...
			// Should not include the appointment with a survey
			const hasAppointmentWithSurvey = response.body.some(
				(survey) => survey.appointmentId === testAppointment.id
			);
			expect(hasAppointmentWithSurvey).toBe(false);
		});

		it("should not allow doctors to access pending surveys", async () => {
			const response = await request(app)
				.get("/api/surveys/pending")
				.set("x-auth-token", doctorToken)
				.expect(403);
	...
	describe("GET /api/surveys/my-stats", () => {
		beforeEach(async () => {
		...
		it("should get survey statistics for patient", async () => {
			const response = await request(app)
				.get("/api/surveys/my-stats")
				.set("x-auth-token", patientToken)
				.expect(200);

			expect(response.body).toHaveProperty("totalCompletedAppointments");
			expect(response.body).toHaveProperty("surveysCompleted");
			expect(response.body).toHaveProperty("surveysPending");
			expect(response.body).toHaveProperty("completionRate");

			expect(typeof response.body.totalCompletedAppointments).toBe(
				"number"
			);
			expect(typeof response.body.surveysCompleted).toBe("number");
			expect(typeof response.body.surveysPending).toBe("number");
			expect(typeof response.body.completionRate).toBe("number");
		});

		it("should not allow doctors to access patient stats", async () => {
			const response = await request(app)
				.get("/api/surveys/my-stats")
				.set("x-auth-token", doctorToken)
				.expect(403);
	...
	describe("GET /api/surveys/stats", () => {
		...
		it("should get survey statistics as admin", async () => {
			...
			expect(response.body.averageRatings).toHaveProperty(
				"appointmentEase"
			);
