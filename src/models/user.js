...
const User = {
  ...
  createPatient: async (userId, patientData = {}) => {
    const { dateOfBirth, phone, address, emergencyContact, emergencyPhone } = patientData;
    ...
    const values = [userId, dateOfBirth || null, phone || null, address || null, 
  ...
  createDoctor: async (userId, doctorData = {}) => {
    ...
    const values = [userId, specialty || null, licenseNumber || null, 
                    phone || null, workSchedule || null];
    const result = await db.query(query, values);
